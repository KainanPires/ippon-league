...
export default function ProMaxCentral() {
  ...
  return (
    ...
    <a href="/inicio" aria-label={t("comum.voltar")} style={{ width: 34, height: 34, borderRadius: "50%", border: "1px solid #24364a", display: "flex", alignItems: "center", justifyContent: "center", color: "#9fb3cc", textDecoration: "none", flexShrink: 0 }}>
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M15 18l-6-6 6-6" /></svg>
    </a>
    <h1 style={{ fontFamily: FD, fontSize: 18, fontWeight: 700, textTransform: "uppercase", margin: 0 }}>{t("pro.centralMax")}</h1>
    </header>
    ...
    {verBoasVindas && (
        ...
        <button onClick={fecharBoasVindas} aria-label={t("comum.fechar")} style={{ position: "absolute", top: 10, right: 10, width: 26, height: 26, borderRadius: "50%", border: "1px solid #2f5478", background: "rgba(0,0,0,0.25)", color: MAX, cursor: "pointer", fontSize: 13, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
        <div style={{ width: 72, height: 72, margin: "0 auto 6px" }}><Mascot belt="#141110" expression="comemorando" /></div>
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", color: MAX }}>★ {t("pro.membroMax")} ★</div>
        <h2 style={{ fontFamily: FD, fontSize: 20, fontWeight: 700, textTransform: "uppercase", margin: "4px 0 6px" }}>{t("pro.olaNome", { nome })}</h2>
        <p style={{ fontSize: 13.5, color: "#9fb3cc", lineHeight: 1.55, margin: 0 }}>{t("pro.mxPacoteCompleto")}</p>
        </section>
      )}
    ...
    <div style={{ fontFamily: FD, fontSize: 15, fontWeight: 700, textTransform: "uppercase", color: MAX }}>{t("pro.exChaveAoVivoT")}</div>
    <div style={{ fontSize: 12, color: "#9fb3cc", marginTop: 1, lineHeight: 1.4 }}>{t("pro.mxChaveAoVivoCard")}</div>
    </div>
    ...
    {LINK_COMUNIDADE && (
        ...
        <div style={{ fontSize: 12, color: "#9fc9ae", marginTop: 1, lineHeight: 1.4 }}>{t("pro.comunidadeSub")}</div>
        </div>
    ...
    {verVantagens && (
        ...
        <button onClick={fecharVantagens} aria-label={t("comum.fechar")} style={{ position: "absolute", top: 10, right: 10, width: 24, height: 24, borderRadius: "50%", border: "1px solid #2f5478", background: "transparent", color: "#9fb3cc", cursor: "pointer", fontSize: 12, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
        <div style={{ fontFamily: FD, fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: MAX, marginBottom: 12 }}>{t("pro.mxVantagensTit")}</div>
    ...
    {/* PERSONALIZAÇÃO */}
    <div style={{ fontFamily: FD, fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: MAX, marginBottom: 10 }}>{t("pro.mxPersonalizacao")}</div>
...
function SeletorTatameCentral() {
  ...
  return (
    ...
    <span style={{ fontSize: 13, fontWeight: 700 }}>{t("pro.mxCorTatame")}</span>
    </span>
    ...
    {aberto && (
        ...
        {TATAMES.map((tt) => {
              const escolhido = isProMax && tt.id === tatameId;
              return (
                <button key={tt.id} onClick={() => { if (isProMax) void setTatame(tt.id as TatameId); }} style={{ position: "relative", display: "flex", flexDirection: "column", gap: 6, background: "transparent", border: `2px solid ${escolhido ? "#7fd1a3" : "#24364a"}`, borderRadius: 12, padding: 7, cursor: "pointer" }}>
...
function SeletorJudoguiCentral() {
  ...
  return (
    ...
    <span style={{ fontSize: 13, fontWeight: 700 }}>{t("pro.mxCorJudogui")}</span>
    </span>
    <span style={{ color: "#9fb3cc", transform: aberto ? "rotate(90deg)" : "none", transition: "transform 0.2s" }}>
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M9 18l6-6-6-6" /></svg>
    </span>
