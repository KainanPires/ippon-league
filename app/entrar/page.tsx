...
export default function Entrar() {
  // O PRIMEIRO ECRÃ TRADUZIDO.
  //
  // Começa por aqui de propósito: é a porta. Alguém que não fale português
  // batia contra ela e ia-se embora sem chegar a ver o jogo. O seletor de
  ...
  async function entrar() {
    ...
    if (!email.trim() || !senha.trim()) {
      setErro(t("entrar.preencher"));
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setErro(t("erro.emailInvalido"));
      return;
    }
    if (!supabaseConfigured) {
      setErro(t("entrar.servidor"));
      return;
    }
    setErro("");
    setRecEnviado(false);
    ...
    if (error) {
      // As mensagens do Supabase vêm SEMPRE em inglês. Não se mostram à pessoa:
      // usam-se só para escolher a nossa mensagem, essa sim traduzida.
      const msg = error.message || "";
      if (/invalid login credentials/i.test(msg)) {
        setErro(t("entrar.credenciaisErradas"));
      } else if (/email not confirmed/i.test(msg)) {
        setErro(t("entrar.naoConfirmado"));
      } else {
        setErro(t("entrar.falhou"));
      }
      setSaving(false);
      return;
    }
  ...
  async function recuperar() {
    ...
    setRecEnviado(false);
    if (!email.trim()) {
      setErro(t("entrar.recEscreveEmail"));
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setErro(t("erro.emailInvalido"));
  ...
  return (
    <main style={{ minHeight: "100vh", background: "radial-gradient(circle at 50% 0%, #143026 0%, #0c0e0d 58%)", color: "#f1ede2", fontFamily: FB, display: "flex", alignItems: "center", justifyContent: "center", padding: "28px 18px" }}>
      <div style={{ width: "100%", maxWidth: 380 }}>
        ...
        {/* O SELETOR ANTES DE TUDO. Quem não percebe a língua do ecrã tem de
            poder trocá-la sem ler nada — daí as bandeiras, sem texto. */}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 9, justifyContent: "center", marginBottom: 4 }}>
          ...
          {/* "Ippon League" é o nome do produto: não se traduz em língua nenhuma. */}
          <span style={{ fontFamily: FD, fontSize: 23, fontWeight: 700, textTransform: "uppercase" }}>
            Ippon <span style={{ color: BLUE }}>League</span>
          </span>
        </div>

        <div style={{ textAlign: "center", fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#7c8a82", marginBottom: 14 }}>
          {t("entrar.slogan")}
        </div>
        ...
        <div style={{ background: "#121815", border: "1px solid #243029", borderRadius: 18, padding: 22 }}>
          <h1 style={{ fontFamily: FD, fontSize: 22, fontWeight: 700, textTransform: "uppercase", textAlign: "center", margin: "0 0 18px" }}>
            {t("entrar.dojo")}
          </h1>

          <Label>{t("entrar.email")}</Label>
          <input
            value={email}
          ...
          {aviso && (
            <div style={{ background: "#2a2410", border: "1px solid #5a4a18", borderRadius: 9, padding: "9px 11px", margin: "-6px 0 12px", fontSize: 12.5, color: "#e8d9a8", lineHeight: 1.45 }}>
              {/* A mensagem do aviso vem do lib/emailSugestao, ainda em
                  português. Fica para quando esse ficheiro for traduzido — o
                  recurso ao português é o comportamento previsto, não um erro. */}
              {aviso.mensagem}
              {aviso.corrigido && (
                <button
                ...
                >
                  {t("entrar.simCorrigir")}
                </button>
              )}
            </div>
          )}

          <Label>{t("entrar.senha")}</Label>
          <div style={{ position: "relative" }}>
            ...
            <button
              ...
              aria-label={showPw ? t("entrar.esconderSenha") : t("entrar.mostrarSenha")}
              style={{ position: "absolute", right: 8, top: 8, width: 32, height: 32, background: "transparent", border: "none", color: "#93a39a", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
            >
          ...
          <div style={{ textAlign: "right", margin: "8px 0 4px" }}>
            <button
              onClick={recuperar}
              disabled={saving}
              style={{ background: "transparent", border: "none", fontSize: 12, color: GOLD, cursor: saving ? "default" : "pointer", fontFamily: FB, padding: 0 }}
            >
              {t("entrar.esqueci")}
            </button>
          </div>
          ...
          {recEnviado && (
            <div style={{ fontSize: 12, color: "#7fd1a3", margin: "4px 0 8px", lineHeight: 1.45 }}>
              {t("entrar.recEnviado", { email: email.trim() })}
            </div>
          )}
          ...
          <button
            onClick={entrar}
            disabled={saving}
            style={{ width: "100%", marginTop: 8, background: GOLD, color: "#1b211e", border: "none", fontFamily: FD, fontSize: 16, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.04em", padding: 14, borderRadius: 12, cursor: saving ? "default" : "pointer", opacity: saving ? 0.7 : 1 }}
          >
            {saving ? t("entrar.aProcessar") : t("entrar.titulo")}
          </button>
        </div>

        <div style={{ textAlign: "center", fontSize: 13, color: "#93a39a", marginTop: 18 }}>
          {t("entrar.semConta")}{" "}
          <a href={`/comecar${voltarQS}`} style={{ color: "#f1ede2", fontWeight: 700, textDecoration: "none", borderBottom: `2px solid ${GOLD}`, paddingBottom: 1 }}>
            {t("entrar.criarConta")}
          </a>
        </div>
      </div>
