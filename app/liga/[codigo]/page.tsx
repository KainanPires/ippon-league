...
// Recebe o tradutor: função pura, não componente.
...
//
// Recebe o tradutor e o nome do mês por argumento: é uma função pura, não um
...
// ("Esta liga " + verbo + " de ") daria frases sem sentido noutras línguas, onde
...
// Idem: recebe o tradutor e o nome do mês.
function rotuloRodadaItem(s: SemanaCalendario, t: (k: string, v?: Record<string, string | number>) => string, nomeMes: (m: number) => string): string {
  ...
  {!aRedirecionar && estado === "pronto" && liga && (
      ...
      {liga.formato !== "copa" && (
          ...
          {vista === "rodada" && listaRodadas.length > 0 && (
              ...
              >
              {listaRodadas.map((s) => (
                    <option key={s.idCompeticao} value={s.idCompeticao} style={{ background: "#141a17", color: "#f1ede2" }}>{rotuloRodadaItem(s, t, nomeMes)}</option>
                  ))}
              </select>
