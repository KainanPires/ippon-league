...
function Atletas() {
  ...
  // "Os que mais pontuaram em <comp>" — a competição fica a negrito no meio.
  const mp = t("atletas.maisPontuaram").split("%C%");
  return (
    ...
    <h1 style={{ fontFamily: FD, fontSize: 19, fontWeight: 700, textTransform: "uppercase", margin: 0 }}>{t("atletas.titulo")}</h1>
    </header>
    <div style={{ fontSize: 12, color: "#93a39a", marginBottom: 16, paddingLeft: 45, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
    <span>{mp[0]}<span style={{ color: GOLD, fontWeight: 700 }}>{nomeComp || "—"}</span>{mp[1]}</span>
    {modo === "ao-vivo" && (
    ...
    {loading ? (
        ...
        <div style={{ fontFamily: FD, fontSize: 16, fontWeight: 700, textTransform: "uppercase", margin: "4px 0 8px" }}>{t("atletas.semPontos")}</div>
        <p style={{ fontSize: 13, color: "#c7d0c9", lineHeight: 1.5, margin: 0 }}>
        {t("atletas.semPontosCorpo")}
        </p>
        </div>
      ) : (
        <>
        <div style={{ fontSize: 11, color: "#5f6f67", marginBottom: 10 }}>
        {t("atletas.contagem", { n: rows.length })}
        </div>
...
function Row({ r, verificado, onClick }: { r: RankRow; verificado?: boolean; onClick: () => void }) {
  ...
  return (
    ...
    <div style={{ fontSize: 11, color: "#93a39a" }}>{code3(r.countryIso)}{r.category ? ` · ${r.category}kg` : ""}{r.gender ? ` · ${r.gender === "F" ? t("mt.feminino") : t("mt.masculino")}` : ""}</div>
    </div>
    ...
    {r.pontos >= 0 ? "+" : ""}{r.pontos} {t("comum.pts")}
    </div>
    </button>
  );
}
...
// O `label` guarda a CHAVE de tradução (os termos de judo — Ippon, Waza-ari,
// Yuko, Shido — não se traduzem; só os qualificadores).
function linhasDeAcoes(a: Record<string, number>): { label: string; qtd: number; negativo: boolean }[] {
  const def: { chave: string; label: string; negativo: boolean }[] = [
    { chave: "ippon", label: "atletas.acIppon", negativo: false },
    { chave: "waza", label: "atletas.acWaza", negativo: false },
    { chave: "yuko", label: "atletas.acYuko", negativo: false },
    { chave: "shido_provocado", label: "atletas.acShidoProvocado", negativo: false },
    { chave: "ippon_sof", label: "atletas.acIpponSof", negativo: true },
    { chave: "waza_sof", label: "atletas.acWazaSof", negativo: true },
    { chave: "yuko_sof", label: "atletas.acYukoSof", negativo: true },
    { chave: "shido_sof", label: "atletas.acShidoSof", negativo: true },
  ];
...
function Detalhe({ r, nomeComp, verificado, onVerificado, onClose }: { r: RankRow; nomeComp: string; verificado?: boolean; onVerificado: (id: string) => void; onClose: () => void }) {
  ...
  // "Pontuação em <comp>, ..." — a competição a negrito no meio.
  const pe = t("atletas.pontuacaoEm").split("%C%");
  return (
    ...
    <div style={{ fontSize: 10, color: "#93a39a", textTransform: "uppercase" }}>{t("atletas.posicao")}</div>
    ...
    <div style={{ fontSize: 10, color: "#93a39a", textTransform: "uppercase" }}>{t("atletas.pontosRodada")}</div>
    ...
    {(temLutas || temVar) && (
        ...
        {temLutas && (
            <div style={{ flex: 1, background: "#141a17", border: "1px solid #243029", borderRadius: 12, padding: "12px", textAlign: "center" }}>
            <div style={{ fontSize: 10, color: "#93a39a", textTransform: "uppercase" }}>{t("atletas.lutas")}</div>
            ...
            <span style={{ fontFamily: FD, fontSize: 13, fontWeight: 700, color: "#7fd1a3" }}>{r.vitorias}{t("atletas.vit")}</span>
            <span style={{ fontFamily: FD, fontSize: 13, fontWeight: 700, color: "#ef8d83" }}>{r.derrotas}{t("atletas.der")}</span>
            </div>
            </div>
          )}
        {temVar && (
            <div style={{ flex: 1, background: "#141a17", border: "1px solid #243029", borderRadius: 12, padding: "12px", textAlign: "center" }}>
            <div style={{ fontSize: 10, color: "#93a39a", textTransform: "uppercase" }}>{t("atletas.valorizacao")}</div>
    ...
    {pe[0]}<span style={{ color: "#cfd8d2" }}>{nomeComp}</span>{pe[1]}
    </div>
    ...
    {r.acoes && Object.keys(r.acoes).length > 0 && linhasDeAcoes(r.acoes).length > 0 && (
        ...
        {linhasDeAcoes(r.acoes).map((l, i) => (
              ...
              <span style={{ fontSize: 12.5, color: "#d6ddd6" }}>{t(l.label)}</span>
...
function SeloVerificado() {
  const t = useT();
  return (
    <span title={t("atletas.perfilVerificado")} style={{ flexShrink: 0, display: "inline-flex", alignItems: "center", justifyContent: "center", width: 15, height: 15, borderRadius: "50%", background: "#2f6fb3", color: "#fff" }}>
...
function Reivindicar({ r, verificado, onVerificado }: { r: RankRow; verificado: boolean; onVerificado: (id: string) => void }) {
  ...
  async function pedir() {
    ...
    try {
      ...
      if (!j?.ok) { setErro(String(j?.erro || t("atletas.erroPedido"))); return; }
      setFase("aguardar");
    } catch { setErro(t("atletas.erroPedido")); }
  ...
  async function novoCodigo() {
    ...
    try {
      ...
      if (!j?.ok) { setErro(String(j?.erro || t("atletas.erroCodigoNovo"))); return; }
      setExpirado(false); setCodigo("");
      setErro(t("atletas.codigoNovoEnviado"));
    } catch { setErro(t("atletas.erroCodigoNovo")); }
    finally { setAEnviar(false); }
  }
  async function verificar() {
    ...
    try {
      ...
      if (!j?.ok) {
        ...
        setErro(String(j?.erro || t("atletas.codigoErrado")));
        return;
      }
      setFase("feito");
      onVerificado(r.id);
    } catch { setErro(t("atletas.erroVerificar")); }
    finally { setAEnviar(false); }
  }
  ...
  if (fase === "feito") {
    return (
      ...
      <SeloVerificado /> {t("atletas.perfilVerificado")}
      </div>
  ...
  return (
    ...
    {temSessao === false ? (
        ...
        {t("atletas.precisaConta")}
        </p>
        <a href="/entrar?voltar=/atletas" style={{ display: "block", textAlign: "center", background: "#2f6fb3", color: "#fff", fontFamily: FD, fontSize: 13, fontWeight: 700, textTransform: "uppercase", padding: "10px", borderRadius: 9, textDecoration: "none" }}>{t("entrar.titulo")}</a>
        </>
      ) : fase === "aguardar" ? (
        <>
        <div style={{ fontFamily: FD, fontSize: 12, fontWeight: 700, textTransform: "uppercase", color: "#7fb8f5", marginBottom: 6 }}>{t("atletas.pedidoRegistado")}</div>
        <p style={{ fontSize: 12.5, color: "#cdd9e6", lineHeight: 1.5, margin: "0 0 12px" }}>
        {t("atletas.aguardarCorpo")}
        <span style={{ color: "#7c8a82" }}> {t("atletas.codigo48h")}</span>
        </p>
        {expirado && (
        ...
        {aEnviar ? t("atletas.aVerificar") : t("atletas.confirmarCodigo")}
        </button>
        ...
        {t("atletas.mudarInstagram")}
        </button>
        </>
      ) : (
        <>
        <p style={{ fontSize: 12.5, color: "#cdd9e6", lineHeight: 1.5, margin: "0 0 12px" }}>
        {t("atletas.formaCorpo")}
        </p>
        ...
        placeholder={t("atletas.phInstagram")}
...
function Carregando({ inline }: { inline?: boolean }) {
  ...
  const box = (
    ...
    <div style={{ fontFamily: FD, fontSize: 13, letterSpacing: "0.14em", textTransform: "uppercase" }}>{t("atletas.aCarregarRanking")}</div>
    </div>
  );
