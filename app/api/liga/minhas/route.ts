...
export async function GET(req: Request) {
  ...
  const { data: ligas } = await supabaseAdmin
    .from("leagues")
    .select("id, name, type, scope, formato, privacidade, descricao, escudo, invite_code, created_by, created_at, estado, copa_estado")
    .in("id", ids);
  ...
  const saida = (ligas || []).map((l) => ({
    ...
    estado: l.estado ?? "ativa",
    copa_estado: l.copa_estado ?? null,
  }));
