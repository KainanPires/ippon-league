...
export async function GET(req: Request) {
  ...
  const debug = searchParams.get("debug") === "1"; // ?debug=1 → lista os pesos existentes
  if (!comp || (!cat && !debug)) {
  ...
  // MODO DEBUG: devolve a lista de pesos (weight) distintos que existem nesta
  // competição, com a contagem de lutas de cada um. Serve para confirmar o
  // formato EXATO do campo weight (ex.: "-48" vs "48" vs "-48 kg") e ajustar a
  // lista de categorias da página. Remover depois de confirmar.
  if (debug) {
    const cont = new Map<string, number>();
    for (const c of todos as Bruto[]) {
      const w = s(c["weight"]);
      cont.set(w, (cont.get(w) || 0) + 1);
    }
    const pesos = Array.from(cont.entries())
      .map(([weight, lutas]) => ({ weight, lutas }))
      .sort((a, b) => b.lutas - a.lutas);
    return NextResponse.json({ ok: true, comp, total_lutas: (todos as unknown[]).length, pesos });
  }
