...
// Estado do jogador (mais tarde vem do Supabase). false = ainda não montou equipa.
const HAS_TEAM = false;

const USER = { name: "Kainan Pires", belt: "Roxa" };
...
export default function Inicio() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#0c0e0d", color: "#f1ede2", fontFamily: FB }}>
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "16px 14px 86px" }}>
        ...
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9, background: "#141a17", border: "1px solid #243029", borderRadius: 999, padding: "5px 14px 5px 5px" }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, lineHeight: 1.1 }}>{USER.name}</div>
              <div style={{ fontSize: 11, color: GOLD }}>Faixa {USER.belt}</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setStep(0)} aria-label="Como se joga" style={iconBtn}>?</button>
        ...
        <Card>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10 }}>
            ...
            <a href="/meu-time" style={{ background: "#1c3a2e", color: "#aee9c9", fontSize: 11, fontWeight: 700, padding: "6px 12px", borderRadius: 8, textDecoration: "none" }}>Escalar</a>
          </div>
        </Card>
      ...
      {/* Tutorial — camada transparente por cima */}
      {step !== null && <Tutorial step={step} setStep={setStep} />}
    </main>
  );
}
  ...
  return (
    <div style={{ border: "1px solid #2a4d3e", borderRadius: 16, overflow: "hidden", marginBottom: 14, background: "repeating-linear-gradient(45deg,#1c3a2e 0 16px,#1a352a 16px 32px)" }}>
      <div style={{ padding: "20px 16px", textAlign: "center" }}>
        ...
        <a href="/meu-time" style={{ display: "block", background: GOLD, color: "#1b211e", fontFamily: FD, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.04em", padding: 13, borderRadius: 11, fontSize: 15, textDecoration: "none" }}>
...
function Tutorial({ step, setStep }: { step: number; setStep: (s: number | null) => void }) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(6,8,7,0.78)", display: "flex", alignItems: "center", justifyContent: "center", padding: 18, zIndex: 100 }}>
      <div style={{ width: "100%", maxWidth: 320 }}>
        <div style={{ textAlign: "right", marginBottom: 8 }}>
          <button onClick={() => setStep(null)} style={{ background: "transparent", border: "none", color: "#cfd8d2", fontSize: 12, cursor: "pointer", fontFamily: FB }}>
        ...
        {!isPro ? (
          ...
          <div style={{ background: "#121815", border: `1px solid ${GOLD}`, borderRadius: 16, padding: 18, textAlign: "center" }}>
            ...
            <div style={{ background: GOLD, color: "#1b211e", fontFamily: FD, fontWeight: 700, textTransform: "uppercase", padding: 11, borderRadius: 10, fontSize: 14 }}>Quero o Ippon Pro</div>
            <button onClick={() => setStep(null)} style={{ marginTop: 10, background: "transparent", border: "none", color: "#93a39a", fontSize: 12, cursor: "pointer", fontFamily: FB }}>Talvez depois</button>
          </div>
        )}
      </div>
    </div>
